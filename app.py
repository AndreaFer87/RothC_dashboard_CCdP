...
st.markdown("""
    <style>
    html, body, [class*="css"] {
        font-size: 18px !important; /* Aumenta il font base */
    }
    .stSelectbox label, .stRadio label, .stMultiSelect label {
        font-size: 20px !important; /* Font etichette input */
        font-weight: bold !important;
    }
    .stButton button {
        font-size: 22px !important; /* Font bottone simulazione */
        height: 3em !important;
    }
    .stTabs [data-baseweb="tab"] {
        font-size: 20px !important; /* Font dei Tab */
    }
    </style>
...
def apply_final_layout(fig, df_visualizzato, title, baseline_name, punti_riferimento):
    ...
    fig.update_layout(
        ...
        xaxis=dict(range=[pd.to_datetime("2021-01-01"), pd.to_datetime("2031-01-01")], 
                   fixedrange=True, showgrid=False, tickfont=dict(size=14)), 
        yaxis=dict(range=[y_min, y_max], title="Stock di C (ton/ha)", showgrid=False, tickfont=dict(size=14), titlefont=dict(size=18)),
        sliders=[],
        updatemenus=[dict(
...
# --- LIVELLO 2 E 3 (Sistemati con font e bottone singolo) ---
with tab2:
    ...
    if df_si is not None and df_no is not None:
        ...
        with c2: scen2 = st.selectbox("✨ Scenario Rigenerativo", [s for s in df_si['Scenario_Esteso'].unique() if base_n not in s], key="scen2_l2")
        with c3: amm_base = st.radio("Ammendante nella Baseline?", ["Sì", "No"], key="amm_l2")
        ...
        fig2 = px.line(df_anim2, x='Data', y='total_soc', color='Legenda', animation_frame='Frame',
                       color_discrete_map={b_ref_name: "#0000FF"}, template="plotly_white", category_orders={"Legenda": targets2_list})
        fig2 = apply_final_layout(fig2, df_anim2, f"Impatto Ammendante - {p2}", b_ref_name, [(val_2026_l2, "Base")])
        st.plotly_chart(fig2, use_container_width=True, config={'displayModeBar': False})

with tab3:
    c1, c2 = st.columns(2)
    with c1: pa, aa = st.selectbox("Sito A", ["Cremona", "Mantova", "Piacenza"], key="pa_l3"), st.radio("Amm. A", ["Sì", "No"], key="aa_l3")
    with c2: pb, ab = st.selectbox("Sito B", ["Cremona", "Mantova", "Piacenza"], index=1, key="pb_l3"), st.radio("Amm. B", ["Sì", "No"], key="ab_l3")
    dfa, dfb = load_data(pa, aa), load_data(pb, ab)
    if dfa is not None and dfb is not None:
        rot3 = st.selectbox("🚜 Rotazione Comune", list(set(dfa['Rotazione']) & set(dfb['Rotazione'])), key="rot3_l3")
        scen3 = st.selectbox("✨ Scenario da confrontare", [s for s in dfa['Scenario_Esteso'].unique() if base_n not in s], key="scen3_l3")

...
# --- CUSTOM CSS PER AUMENTARE IL FONT ---
st.markdown("""
    <style>
    html, body, [class*="css"] {
...
def load_data(provincia, scelta_amm):
    ...
    try:
        # Nota: assicurarsi che openpyxl sia installato
        ...
        st.error(f"Errore caricamento file {file_name}. Assicurati che i file Excel siano nella cartella.")
        return None

def apply_final_layout(fig, df_visualizzato, title, baseline_name, punti_riferimento):
    # SAFETY CHECK: Evita ValueError se i dati sono vuoti
    if df_visualizzato is None or df_visualizzato.empty:
        y_min, y_max = 0, 100
    else:
        y_min = df_visualizzato['total_soc'].min() * 0.98
        y_max = df_visualizzato['total_soc'].max() * 1.02
        if pd.isna(y_min) or pd.isna(y_max): y_min, y_max = 0, 100
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[y_min, y_max], 
                   title=dict(text="Stock di C (ton/ha)", font=dict(size=20)), 
...
with tab1:
    ...
    with c2: a1 = st.radio(f"Uso Ammendanti?", ["Sì", "No"], horizontal=True, key="a1")
    ...
    if df1 is not None:
        ...
        if is_piacenza_cc:
            st.info("💡 Per questa rotazione è disponibile l'analisi delle frequenze Cover Crop.")
            m_cc = st.radio("🧐 **Modalità di Analisi**", ["Standard", "Confronta Frequenze CC"], horizontal=True)

            if m_cc == "Confronta Frequenze CC":
                map_cc = {
                ...
                scen_cc_scelto = st.selectbox("✨ Pratica da testare", [s for s in df1[df1['Rotazione'] == map_cc["CC Anno 1"]]['Scenario_Esteso'].unique() if s != base_n])
                scelte_freq = st.multiselect("📅 Seleziona frequenze", list(map_cc.keys()))
                ...
                if scelte_freq:
                    ...
                    for s in final_targets:
                        if s == base_n: u = df_base_real.copy()
                            ...
                            df_spec = df1[(df1['Rotazione'] == map_cc[s]) & (df1['Scenario_Esteso'] == scen_cc_scelto)].copy()
        ...
        if not df_merged_scenarios.empty and len(final_targets) > 1:
            # Calcolo val_2026 in modo sicuro
            try:
...
# --- LIVELLO 2 E 3 (Semplificati e sicuri) ---
with tab2:
    p2 = st.selectbox("📍 Provincia", ["Cremona", "Mantova", "Piacenza"], key="p2_l2")
    df_si, df_no = load_data(p2, "Sì"), load_data(p2, "No")
    if df_si is not None and df_no is not None:
        c1, c2, c3 = st.columns(3)
        with c1: rot2 = st.selectbox("🚜 Rotazione", df_si['Rotazione'].unique(), key="rot2_l2")
        with c2: scen2 = st.selectbox("✨ Scenario", [s for s in df_si['Scenario_Esteso'].unique() if base_n not in s], key="scen2_l2")
        with c3: amm_base = st.radio("Ammendante in Baseline?", ["Sì", "No"], key="amm_l2")
        
        df_base_ref = df_si if amm_base == "Sì" else df_no
        b_ref_name = "Baseline (Riferimento)"
        t2_list = [f"{scen2} (+ Amm.)", f"{scen2} (No Amm.)", b_ref_name]
        ...
        for m in range(1, 118, 4):
            for t in t2_list:
                ...
                else: temp = src[(src['Rotazione'] == rot2) & (src['Scenario_Esteso'] == sn) & (src['Mese_Progressivo'] <= m)].copy()
                temp['Legenda'], temp['Frame'] = t, m
                anim2.append(temp)
        
        df_anim2 = pd.concat(anim2)
        try: val_2026_l2 = df_base_ref[(df_base_ref['Rotazione'] == rot2) & (df_base_ref['Scenario_Esteso'] == base_n) & (df_base_ref['Mese_Progressivo'] == 61)]['total_soc'].values[0]
        except: val_2026_l2 = None
...
with tab3:
    st.subheader("🌍 Confronto tra diversi territori")
    c1, c2 = st.columns(2)
    ...
    if dfa is not None and dfb is not None:
        ...
        fig3 = apply_final_layout(fig3, df_anim3, "Confronto Territoriale", "NESSUNA", [])

...
# Configurazione pagina larga
st.set_page_config(page_title="Casalasco Decarb - Pro", layout="wide")

# --- PARAMETRO UNICO PER IL FONT ---
FONT_SIZE = 20 

# --- CSS PER ESPANDERE TUTTO LO SCHERMO E AUMENTARE I FONT ---
st.markdown(f"""
    <style>
    /* Forza il contenitore principale a occupare il 98% della larghezza */
    .block-container {{
        padding-top: 1.5rem;
        padding-bottom: 0rem;
        padding-left: 2rem;
    ...
    /* Font dei selettori (Selectbox, Radio, Multiselect) e Tab */
    .stSelectbox label, .stRadio label, .stMultiSelect label, .stTabs [data-baseweb="tab"] p {{
        font-size: {FONT_SIZE}px !important;
        font-weight: bold !important;
    }}
    /* Dimensione testo nei pulsanti radio orizzontali */
...
def apply_final_layout(fig, df_visualizzato, title, baseline_name, punti_riferimento):
    ...
    fig.update_layout(
        height=700,
        margin=dict(l=0, r=0, t=60, b=50), # Margini azzerati per massima larghezza
        title=dict(text=title, font=dict(size=FONT_SIZE+6), x=0),
        xaxis=dict(
        ...
        # Legenda orizzontale in alto per non stringere il grafico
        legend=dict(font=dict(size=FONT_SIZE-4), orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        sliders=[], 
        updatemenus=[dict(
    ...
    fig.update_traces(line=dict(width=1.5), selector=dict(name=baseline_name)) # Baseline molto sottile
...
with tab1:
    ...
    if df1 is not None:
        ...
        scen_scelti = st.multiselect("✨ Seleziona Scenari", [s for s in df1[df1['Rotazione'] == rot1]['Scenario_Esteso'].unique() if s != base_n], key="multi1")
        final_targets = scen_scelti + [base_n]
        
        if len(final_targets) > 1:
            df_m = pd.concat([pd.concat([df_base_real[df_base_real['Mese_Progressivo'] <= 60], df1[(df1['Rotazione'] == rot1) & (df1['Scenario_Esteso'] == s)][df1['Mese_Progressivo'] > 60]]).assign(Legenda=s) for s in final_targets]).sort_values('Mese_Progressivo')
            val_2026 = df_base_real[df_base_real['Mese_Progressivo'] == 61]['total_soc'].values[0]
            anim_frames = []
            for m in range(1, 118, 4):
                for s in final_targets:
                    temp_f = df_base_real[df_base_real['Mese_Progressivo'] <= m].copy() if m <= 60 else df_m[(df_m['Legenda'] == s) & (df_m['Mese_Progressivo'] <= m)].copy()
            ...
            fig1 = apply_final_layout(fig1, df_m, f"Proiezione Carbonio - {p1}", base_n, [(val_2026, p1)])
